...
export class categoryController{
    ...
    static addCategory  = async (req, res) => {
        const {categoryName} = req.body
           try {
               ...
               if (data) res.status(200).send({message: "categoryName  Successfully add"})
               else return res.status(400).send({message: "Add categoryName Error In Database"})
           } catch (error)
            {
    ...
    static getCategory = async (req, res) => {
        ...
        try {
           ...
           const data = await categoryModel.find() 
            if (data) res.status(200).send({Message: "category Successfully Get",user:(data)})
    ...
    static updateCategory = async (req, res) => {
        ...
        try {
            ...
            if (response) {
                return res.status(200).send({message: "category Successfully update"})
            }
